...
class PCContext(EnvContext):
    """Context embedding for Part Consolidation.

    The decoder state is summarized by the last selected token and the average
    embedding of the current open group. Empty groups contribute a zero vector.
    """

    def __init__(self, embed_dim):
        super().__init__(embed_dim=embed_dim, step_context_dim=2 * embed_dim)

    def forward(self, embeddings, td):
        node_dim = embeddings.dim() - 2
        cur_node_embedding = gather_by_index(
            embeddings, td["current_node"].long(), dim=node_dim
        )

        group_mask = td["current_group_mask"].float().unsqueeze(-1)
        group_embeddings = embeddings
        while group_embeddings.dim() < group_mask.dim():
            group_embeddings = group_embeddings.unsqueeze(1)
        group_count = group_mask.sum(dim=-2).clamp_min(1.0)
        group_embedding = (group_embeddings * group_mask).sum(dim=-2) / group_count

        context_embedding = torch.cat([cur_node_embedding, group_embedding], dim=-1)
        return self.project_context(context_embedding)
